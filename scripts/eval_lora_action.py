...
def main():
    ...
    # Load LoRA for selected action (patched: strict fallback)
    load_lora_into_toolbank(tb, cfg.action_internal, cfg.lora_ckpt)
    tb.activate(cfg.action_internal, cfg.scale)
    print(f"[ToolBank] activate(action={cfg.action_internal}, scale={cfg.scale})")

    tb.eval()
    backbone.eval()

    out_dir = os.path.join(cfg.results_root, cfg.action_alias, f"{cfg.split}_{cfg.split_mode}")
    ...
    pbar = tqdm(loader, ncols=140, desc=f"Eval {cfg.action_alias}/{cfg.split}")
    with torch.no_grad():
        for it, (inp, gt, metas) in enumerate(pbar, start=1):
            ...
            # ------------------------------------------------------------
            # Sanity checks on first iter (TRUE backbone-only vs toolbank)
            # ------------------------------------------------------------
            if it == 1:
                # 1) pairing sanity
                ...
                print(f"[Sanity] PSNR(input,gt)={ps_in_gt:.2f} SSIM(input,gt)={ss_in_gt:.4f}")
                print("[Sanity] meta0 =", metas[0])

                # 2) TRUE backbone-only sanity:
                #    ToolBank wraps backbone convs; so to get "backbone-only", force LoRA scale=0.
                tb.activate(cfg.action_internal, 0.0)
                ...
                print(f"[Sanity] PSNR(backbone(scale=0),gt)={ps_bb:.2f} SSIM={ss_bb:.4f}")

                # 3) toolbank pred sanity (with requested scale)
                tb.activate(cfg.action_internal, cfg.scale)
                with autocast(device_type="cuda", dtype=torch.float16, enabled=(cfg.use_amp and device.type == "cuda")):
                    pred_pad = tb(inp_pad)
                ...
                print(f"[Sanity] PSNR(pred(scale={cfg.scale}),gt)={ps_pred:.2f} SSIM={ss_pred:.4f}")

                # 4) pred vs bb delta (proof LoRA is doing something)
                d_mean = (pred - bb).abs().mean().item()
                d_max = (pred - bb).abs().max().item()
                print(f"[Sanity] |pred-bb| mean={d_mean:.6e} max={d_max:.6e}")

                # 5) range/shape prints
                mn, mx = _tensor_range(inp)
                print(f"[Sanity] inp shape/range: {tuple(inp.shape)} {mn} {mx}")
                mn, mx = _tensor_range(bb)
                print(f"[Sanity] bb  shape/range: {tuple(bb.shape)} {mn} {mx}")
                mn, mx = _tensor_range(pred)
                print(f"[Sanity] pred shape/range: {tuple(pred.shape)} {mn} {mx}")

                # NOTE: we already computed pred for it==1, so skip normal forward below
            else:
                # ---------------------------
                # normal toolbank forward
                # ---------------------------
                with autocast(device_type="cuda", dtype=torch.float16, enabled=(cfg.use_amp and device.type == "cuda")):
                    pred_pad = tb(inp_pad)
                pred = unpad(pred_pad, pad).clamp(0, 1)
    ...
    print("\n====================")
    print(f"[DONE] action={cfg.action_alias} split={cfg.split} split_mode={cfg.split_mode}")
    print(f"[AVG] PSNR={psnr_sum/max(1,cnt):.4f}  SSIM={ssim_sum/max(1,cnt):.6f}  (N={cnt})")
    print("[OUT] ", out_dir)
    print("====================\n")
