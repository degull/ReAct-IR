...
# ============================================================
# Action / Label mapping  (★ run_agent_llma.py와 반드시 동일해야 함)
# ============================================================
...
# ============================================================
# Score config (★ s0 saturation 완화용: logits -> score 변환을 "ckpt에 저장"해둠)
# - 학습 loss/metrics는 logits 기반(BCEWithLogits) 그대로.
# - Planner/Agent에서만 아래 규칙으로 s0 만들면 포화가 줄고 "순위+세기"가 살아남.
# ============================================================
SCORE_NORM_MODE_DEFAULT = "centered"  # plain | centered | zscore
SCORE_TEMPERATURE_DEFAULT = 1.8
SCORE_EPS_DEFAULT = 1e-6
...
# ---------------------------
# s0 saturation 완화용 변환들
# (학습에는 영향 없음. agent/planner에서 그대로 쓰면 됨)
# ---------------------------
def scores_from_logits(logits: torch.Tensor, temperature: float = SCORE_TEMPERATURE_DEFAULT, eps: float = SCORE_EPS_DEFAULT):
    T = max(float(temperature), 1e-6)
    s = torch.sigmoid(logits / T)
    return s.clamp(eps, 1.0 - eps)


def scores_from_logits_centered(logits: torch.Tensor, temperature: float = SCORE_TEMPERATURE_DEFAULT, eps: float = SCORE_EPS_DEFAULT):
...
def scores_from_logits_zscore(logits: torch.Tensor, temperature: float = 1.0, eps: float = SCORE_EPS_DEFAULT):
...
def build_state_from_outputs(
    logits: torch.Tensor,
    m0: torch.Tensor,
    temperature: float = SCORE_TEMPERATURE_DEFAULT,
    norm_mode: str = SCORE_NORM_MODE_DEFAULT,
    eps: float = SCORE_EPS_DEFAULT,
):
    """
    logits: (B,5)
    m0:     (B,5,h,w)
    ...
    returns:
      s0: (B,5)  (temperature+norm 적용된 점수)
      m0_mean: (B,5)
      m0_max:  (B,5)  <-- 채널별 max (H,W만 reduce)
    """
    # ✅ m0 stats: channel-wise reduce over (H,W) only
    m0_mean = m0.mean(dim=(2, 3))
    m0_max = m0.amax(dim=(2, 3))

    if norm_mode == "plain":
        s0 = scores_from_logits(logits, temperature=temperature, eps=eps)
    elif norm_mode == "centered":
        s0 = scores_from_logits_centered(logits, temperature=temperature, eps=eps)
    elif norm_mode == "zscore":
        s0 = scores_from_logits_zscore(logits, temperature=temperature, eps=eps)
    else:
...
def multilabel_metrics_from_logits(logits: torch.Tensor, targets: torch.Tensor, thr: float = 0.5) -> Dict[str, float]:
    ...
    주의:
      - metrics는 "기본 sigmoid" 기준으로 유지 (학습 안정성/비교 일관성)
      - 포화 완화 score(s0)는 planner/agent에서만 쓰는 걸 추천
    """
...
def train_one_epoch(
    ...
    for it, (inp, y, metas) in enumerate(pbar, start=1):
        ...
        with torch.no_grad():
            ...
            # --- saturation 감시(디버깅) ---
            probs_plain = torch.sigmoid(logits.detach())
            sat_rate = ((probs_plain > 0.99) | (probs_plain < 0.01)).float().mean().item()
...
def main():
    ...
    print("[Action->LabelIndex]", ACTION_TO_LABEL_INDEX)
    print("[Model]", cfg.model)
    print("[ScoreCfg] norm_mode=", SCORE_NORM_MODE_DEFAULT, "temp=", SCORE_TEMPERATURE_DEFAULT, "eps=", SCORE_EPS_DEFAULT)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        ckpt = {
            ...
            # ✅ mapping info를 ckpt에 박아둠 (agent가 읽으면 "순서 꼬임" 방지)
            "actions_internal": ACTIONS_INTERNAL,
            "action_to_label_index": ACTION_TO_LABEL_INDEX,
            "alias_to_internal": ALIAS_TO_INTERNAL,

            # ✅ score config도 같이 저장 (agent에서 s0 saturation 완화용)
            "score_cfg": {
                "norm_mode": SCORE_NORM_MODE_DEFAULT,
                "temperature": SCORE_TEMPERATURE_DEFAULT,
                "eps": SCORE_EPS_DEFAULT,
            },
    ...
    # (선택) agent에서 쓸 때 아래 원칙을 지키면 됨:
    # - logits -> s0 변환은 ckpt["score_cfg"] 기반으로:
    #     centered + temperature (default 1.8)
    # - m0_mean/m0_max는 m0.mean(dim=(2,3)), m0.amax(dim=(2,3)) 로 채널별 계산
